namespace OrganisationRegistry.UnitTests.Organisation.Kbo
{
    ...
    public class UpdateFromKboWithoutLegalFormTests: Specification<Organisation, KboOrganisationCommandHandlers, UpdateFromKbo>
    {
            ...
            return new KboOrganisationCommandHandlers(
                ...
                kboOrganisationRetriever: new KboOrganisationRetrieverStub(
                    new MockMagdaOrganisationResponse
                    {
                        ...
                        ShortName = new NameStub("Formele naam organisatie", new DateTime(2010, 1, 1)),
        ...
        {
            var organisationCoupledWithKbo = PublishedEvents[0].UnwrapBody<OrganisationInfoUpdatedFromKbo>();
            organisationCoupledWithKbo.Should().NotBeNull();

            organisationCoupledWithKbo.OrganisationId.Should().Be(_organisationId);
            organisationCoupledWithKbo.Name.Should().Be("Kbo formele naam");
            organisationCoupledWithKbo.ShortName.Should().Be("Formele naam organisatie");
        }

        [Fact]
        public void UpdatesLegalForms()
        {
            var legalFormOrganisationOrganisationClassificationEnded = PublishedEvents[1].UnwrapBody<KboLegalFormOrganisationOrganisationClassificationRemoved>();
        ...
        {
            var organisationSyncedFromKbo = PublishedEvents[2].UnwrapBody<OrganisationSyncedFromKbo>();
