namespace OrganisationRegistry.Api.IntegrationTests.Security.PermissionMatrix.Organisation.Regulations.When_Updating_Regulations;
...
public class Given_Roles_Without_CanManageRegulations
{
    ...
    public async Task Then_Returns_Forbidden(string role)
    {
        ...
        var response = await ApiFixture.Put(
            client,
            $"/v1/organisations/{organisationId}/regulations/{entityId}",
            new UpdateOrganisationRegulationRequest()
            {
