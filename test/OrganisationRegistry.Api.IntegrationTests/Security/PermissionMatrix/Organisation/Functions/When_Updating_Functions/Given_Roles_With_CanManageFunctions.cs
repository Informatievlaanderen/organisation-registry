namespace OrganisationRegistry.Api.IntegrationTests.Security.PermissionMatrix.Organisation.Functions.When_Updating_Functions;
...
public class Given_Roles_With_CanManageFunctions
{
    ...
    public async Task For_Algemeenbeheerder_Then_Returns_OK()
    {
        ...
        var response = await UpdateFunction(client, organisationId, entityId);
    ...
    private async Task<HttpResponseMessage> UpdateFunction(HttpClient client, Guid organisationId, Guid entityId)
    {
        var functionTypeId = await _apiFixture.Create.Function();
        var personId = await _apiFixture.Create.Person();
