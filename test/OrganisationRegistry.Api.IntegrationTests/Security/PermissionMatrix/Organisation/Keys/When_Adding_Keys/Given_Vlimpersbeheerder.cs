namespace OrganisationRegistry.Api.IntegrationTests.Security.PermissionMatrix.Organisation.Keys.When_Adding_Keys;
...
public class Given_Vlimpersbeheerder
{
    ...
    /// door de vlimpersbeheerder aangemaakt worden.
    /// </summary>
    [Fact]
    public async Task For_Vlimpersbeheerder_WithVlimpersKeyOnVlimpersManagedOrganisation_Then_Returns_Created()
    {
        ...
        var response = await AddKey(client, organisationId, vlimpersKeyTypeId);
    ...
    /// Vlimpers-beheerde organisatie niet beheren.
    /// </summary>
    [Fact]
    public async Task For_Vlimpersbeheerder_WithNonVlimpersKey_Then_Returns_Forbidden()
    {
        ...
        var response = await AddKey(client, organisationId, otherKeyTypeId);

        response.StatusCode.Should().Be(HttpStatusCode.Forbidden);
    }

    /// <summary>
    /// Negatief: zelfs een Vlimpers-sleuteltype mag niet beheerd worden wanneer de
    ...
    /// door de vlimpersbeheerder enkel beheerd worden voor Vlimpers-organisaties.
    /// Positief: op een organisatie onder Vlimpersbeheer.
    /// </summary>
    [Theory]
    [MemberData(nameof(BusinessUnitAndSetIdKeyTypes))]
    public async Task For_Vlimpersbeheerder_WithNewVlimpersKeyOnVlimpersManagedOrganisation_Then_Returns_Created(Guid keyTypeId)
    {
        var organisationId = await CreateVlimpersManagedOrganisation();
        ...
        var response = await AddKey(client, organisationId, keyTypeId);
    ...
    /// mogen niet beheerd worden op een organisatie die niet onder Vlimpersbeheer valt.
    /// </summary>
    [Theory]
    [MemberData(nameof(BusinessUnitAndSetIdKeyTypes))]
    ...
    private async Task<HttpResponseMessage> AddKey(HttpClient client, Guid organisationId, Guid keyTypeId)
