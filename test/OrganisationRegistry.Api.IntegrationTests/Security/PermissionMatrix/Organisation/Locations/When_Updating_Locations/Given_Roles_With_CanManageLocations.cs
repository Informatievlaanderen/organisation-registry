namespace OrganisationRegistry.Api.IntegrationTests.Security.PermissionMatrix.Organisation.Locations.When_Updating_Locations;
...
public class Given_Roles_With_CanManageLocations
{
    ...
    public async Task For_Algemeenbeheerder_Then_Returns_OK()
    {
        ...
        var response = await UpdateLocation(client, organisationId, entityId);
    ...
    private async Task<HttpResponseMessage> UpdateLocation(HttpClient client, Guid organisationId, Guid entityId)
    {
        var locationId = await _apiFixture.Create.Location();
        ...
        return await ApiFixture.Put(
            ...
            new UpdateOrganisationLocationRequest()
            {
                OrganisationLocationId = entityId,
                LocationId = locationId,
                IsMainLocation = false,
