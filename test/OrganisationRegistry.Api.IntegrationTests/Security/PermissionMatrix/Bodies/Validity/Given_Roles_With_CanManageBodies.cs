...
public class Given_Roles_With_CanManageBodies
{
    ...
    public async Task For_Algemeenbeheerder_Then_Returns_Ok()
    {
        ...
        var response = await UpdateBodyValidity(client, bodyId);
    ...
    public async Task For_Decentraalbeheerder_WithOwnOrganisation_Then_Returns_Ok()
    {
        ...
        var response = await BodyPermissionMatrix.WaitUntilAllowed(bodyId, () => UpdateBodyValidity(client, bodyId));
    ...
    private async Task<HttpResponseMessage> UpdateBodyValidity(HttpClient client, Guid bodyId)
