...
public class WhenTheApiStartsUp
{
    ...
    public async Task StatusResourceReturns200()
        => (await _fixture.HttpClient.GetAsync("/v1/status")).StatusCode.Should().Be(HttpStatusCode.OK);

    [Fact]
    public async Task StatusResourceReturnsItsOk()
        => (await (await _fixture.HttpClient.GetAsync("/v1/status")).Content.ReadAsStringAsync()).Should().Be("\"I'm ok!\"");
}
