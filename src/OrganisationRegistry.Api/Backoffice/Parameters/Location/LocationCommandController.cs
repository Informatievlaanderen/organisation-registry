...
[OrganisationRegistryAuthorize(RequiredPermissions = [Permission.ParametersLocationsWrite])]
[ApiController]
...
public class LocationCommandController : OrganisationRegistryCommandController
{
    ...
    public async Task<IActionResult> Post([FromBody] CreateLocationRequest message)
    {
        ...
        return CreatedWithLocation(nameof(LocationController), "Get", new { id = command.Id });
    }
    ...
    public async Task<IActionResult> Put([FromRoute] Guid id, [FromBody] UpdateLocationRequest message)
    {
        ...
        return OkWithLocationHeader(nameof(LocationController), "Get", new { id = internalMessage.LocationId });
    }
}
