...
public class OrganisationBodyController : OrganisationRegistryController
{
    ...
    /// <response code="200">Een lijst van organen voor een organisatie.</response>
    [HttpGet]
    [ProducesResponseType(typeof(List<OrganisationBodyListItem>), StatusCodes.Status200OK)]
    [SwaggerResponseExample(StatusCodes.Status200OK, typeof(OrganisationBodyListExamples))]
    public async Task<IActionResult> Get([FromServices] OrganisationRegistryContext context, [FromServices] ISecurityService securityService, [FromRoute] Guid organisationId)
    {
        ...
        var pagedOrganisations = new OrganisationBodyListQuery(context, organisationId, await securityService.GetUser(User)).Fetch(filtering, sorting, pagination);
