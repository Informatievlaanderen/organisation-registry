...
public class OrganisationBuildingController : OrganisationRegistryController
{
    ...
    [HttpGet]
    [ProducesResponseType(typeof(List<OrganisationBuildingListItem>), StatusCodes.Status200OK)]
    [SwaggerResponseExample(StatusCodes.Status200OK, typeof(OrganisationBuildingListExamples))]
    [ActionName("List")]
    public async Task<IActionResult> Get([FromServices] OrganisationRegistryContext context, [FromServices] ISecurityService securityService, [FromServices] IMemoryCaches memoryCaches, [FromRoute] Guid organisationId)
    {
        ...
        var pagedOrganisations = new OrganisationBuildingListQuery(context, memoryCaches, organisationId, await securityService.GetUser(User)).Fetch(filtering, sorting, pagination);
