...
export class OrganisationAuthorization {
  ...
  public static canAddBodies(organisation, securityInfo) {
    ...
    if (
      securityInfo.hasAnyOfRoles([Role.AlgemeenBeheerder, Role.OrgaanBeheerder])
    )
      return true;
  ...
  public static canRemoveBankAccounts(organisation, securityInfo) {
    if (!securityInfo.isLoggedIn) return false;
  ...
  public static canRemoveFormalFrameworks(organisation, securityInfo) {
    if (!securityInfo.isLoggedIn) return false;
