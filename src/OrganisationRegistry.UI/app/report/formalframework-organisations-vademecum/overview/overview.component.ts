...
export class FormalFrameworkOverviewComponent implements OnInit {
    ...
    private loadFormalFrameworks(event?: PagedEvent) {
        ...
        let formalFrameworks = (event === undefined)
            ? this.formalFrameworkService.getVademecumFormalFrameworks(this.filter, this.currentSortBy, this.currentSortOrder)
            : this.formalFrameworkService.getVademecumFormalFrameworks(this.filter, event.sortBy, event.sortOrder, event.page, event.pageSize);
