...
export class OrganisationFunctionsOverviewComponent
  implements OnInit, OnDestroy
{
  ...
  constructor(
    ...
    this.organisationFunctions =
      new PagedResult<OrganisationFunctionListItem>();
  }
  ...
  private loadFunctions(event?: PagedEvent) {
    this.isLoading = true;
    let organisationFunctions =
      event === undefined
        ? this.organisationFunctionService.getOrganisationFunctions(
            this.organisationId,
            this.filter,
            this.currentSortBy,
    ...
    this.subscriptions.push(
      organisationFunctions
        ...
        .subscribe(
          (items) => {
            if (items) this.organisationFunctions = items;
          },
          (error) =>
  ...
  removeFunction($event: OrganisationFunctionListItem) {
    if (!confirm("Bent u zeker? Deze actie kan niet ongedaan gemaakt worden."))
      return;

    this.subscriptions.push(
      this.organisationFunctionService
        .delete(this.organisationId, $event.organisationFunctionId)
        .subscribe(
          () => this.loadFunctions(),
          (error) =>
