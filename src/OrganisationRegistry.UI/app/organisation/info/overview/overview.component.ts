...
import { Organisation, OrganisationChild } from "services/organisations";
...
export class OrganisationInfoOverviewComponent implements OnInit, OnDestroy {
  public isBusy = true;
  public children: PagedResult<OrganisationChild> =
  ...
  ngOnInit() {
    this.isOrganisationRegistryBeheerder = this.store.requiresOneOfRole(
      Role.AlgemeenBeheerder,
      Role.CjmBeheerder
    );

    let organisationChangedObservable = this.store.organisationChanged;
    let childrenChangedObservable = this.store.organisationChildrenChanged;

    this.subscriptions.push(
      Observable.zip(
        organisationChangedObservable,
        childrenChangedObservable
      ).subscribe((res) => (this.isBusy = false))
    );

    this.subscriptions.push(
      organisationChangedObservable.subscribe((organisation) => {
        if (organisation) {
    ...
    this.subscriptions.push(
      childrenChangedObservable.subscribe((children) => {
        if (children) {
