...
export class BodyParticipationOverviewComponent implements OnInit, OnDestroy {
  ...
  public body: Body;
  public bodyParticipations: PagedResult<BodyParticipationReportListItem> =
    new PagedResult<BodyParticipationReportListItem>();
  public bodyParticipationTotals: BodyParticipationReportTotals =
    new BodyParticipationReportTotals();
  public canManageMep: Observable<boolean>;

  private filter: BodyParticipationReportFilter =
    ...
    this.route.parent.parent.params.forEach((params: Params) => {
      ...
      this.canManageMep = this.oidcService.roles.map((roles) => {
        return (
          ...
          roles.indexOf(Role.AlgemeenBeheerder) !== -1 ||
          roles.indexOf(Role.CjmBeheerder) !== -1
        );
      });

      this.isLoading = true;
      this.subscriptions.push(
        this.bodyService.get(id).subscribe(
          (item) => {
            if (item) this.body = item;
          },
          (error) =>
  ...
  get isNonCompliant() {
    return (
      this.body.hasAllSeatsAssigned &&
      this.bodyParticipationTotals.compliance === Compliance.NONCOMPLIANT
    );
  }

  private loadBodyParticipations(bodyId: string, event?: PagedEvent) {
    this.isLoading = true;
    let bodyParticipations =
      event === undefined
        ? this.bodyParticipationReportService.getParticipationsPerBody(
            bodyId,
            this.filter,
            this.currentSortBy,
  ...
  private loadBodyParticipationTotals(bodyId: string) {
    this.isLoading = true;
    let bodyParticipationTotals =
      this.bodyParticipationReportService.getParticipationsPerBodyTotals(
        bodyId,
        this.filter
      );
