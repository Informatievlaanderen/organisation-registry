...
var apiBase         = Env("API_BASE",         "http://api");
var keycloakBase    = Env("KEYCLOAK_BASE",    "http://keycloak:80");
var keycloakRealm   = Env("KEYCLOAK_REALM",   "wegwijs");
var clientId        = Env("CLIENT_ID",        "cjmClient");
var clientSecret    = Env("CLIENT_SECRET",    "secret");
...
{
    // EditApi: OIDC introspection config — keycloak service binnen K8s cluster
    ["EditApi:Authority"]             = $"{keycloakBase}/realms/{keycloakRealm}",
    ["EditApi:IntrospectionEndpoint"] = $"{keycloakBase}/realms/{keycloakRealm}/protocol/openid-connect/token/introspect",

    // OIDCAuth: voor browser/SPA flows (US1, US3)
    ["OIDCAuth:Authority"]             = $"{keycloakBase}/realms/{keycloakRealm}",
    ["OIDCAuth:JwksUri"]               = $"{keycloakBase}/realms/{keycloakRealm}/protocol/openid-connect/certs",
...
async Task<string> GetKeycloakTokenAsync()
{
    var tokenEndpoint = $"{keycloakBase}/realms/{keycloakRealm}/protocol/openid-connect/token";
    ...
    var requestData = new FormUrlEncodedContent(new[]
    {
        ...
        new KeyValuePair<string, string>("scope", "dv_organisatieregister_cjmbeheerder")
    });
    ...
    if (!response.IsSuccessStatusCode)
    {
        ...
        throw new Exception($"Failed to get token from Keycloak: {response.StatusCode} - {error}");
    }
