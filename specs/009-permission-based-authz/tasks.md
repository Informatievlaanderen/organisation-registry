...
- [x] T007 Create `RolePermissionMap` static class exposing `For(IEnumerable<Role>, ILogger?)` and `For(IEnumerable<Role>, config, logger)` overloads. Encode the full mapping for `AlgemeenBeheerder, VlimpersBeheerder, DecentraalBeheerder, RegelgevingBeheerder, OrgaanBeheerder, Developer, CjmBeheerder, Orafin, AutomatedTask` per data-model.md. Fail-closed + Serilog error (throttled once/role/process) for unknown roles. `AlgemeenBeheerder` and `Developer` get `CanEditAll` (admin bypass retained). `AutomatedTask` maps to `CanRunScheduledJobs` (event-sourcing immutability; scheduled-job migration deferred to T036). `VlimpersBeheerder` granted restricted `CanManageKeys` via `KeyRestrictions.VlimpersManaged(config.Authorization.KeyIdsAllowedForVlimpers)` (config-aware overload). Two overloads: base returns static map; config-aware adds restricted grants per role. Path: `/code/aiv/organisation-registry/src/OrganisationRegistry.Infrastructure/Authorization/RolePermissionMap.cs`.
