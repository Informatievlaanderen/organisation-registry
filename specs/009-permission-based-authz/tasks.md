...
- [x] T007 Create `RolePermissionMap` static class exposing `For(IEnumerable<Role>) : PermissionSet`. Encode the full mapping for `AlgemeenBeheerder, VlimpersBeheerder, DecentraalBeheerder, RegelgevingBeheerder, OrgaanBeheerder, Developer, CjmBeheerder, Orafin` per data-model.md. Fail-closed + Serilog error (throttled once/role/process) for unknown roles. `AlgemeenBeheerder` krijgt granulair elke relevante permission (geen super-permission short-circuit). Do NOT map `AutomatedTask`. Path: `/code/aiv/organisation-registry/src/OrganisationRegistry.Infrastructure/Authorization/RolePermissionMap.cs`.
